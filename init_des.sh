...
# Step 1: Bring up all containers using docker-compose inside docker/
...
for i in $(seq 1 $MAX_RETRIES); do
  if docker exec des_kafka kafka-topics.sh --bootstrap-server localhost:9092 --list >/dev/null 2>&1; then
    echo "Kafka is ready!"
    break
  else
